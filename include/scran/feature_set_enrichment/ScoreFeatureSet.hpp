 ...
 *
 * For multi-block analyses, we extend this approach by computing the rotation vector from the residuals generated by centering each block.
 * (This is equivalent to the residual-based block policies described in `MultiBatchPca::BlockPolicy`.)
...
private:
    ...
    void compute_scores(const Eigen::MatrixXd& pcs, double multiplier, double shift, std::vector<double>& scores) const {
        scores.resize(pcs.cols());
        for (Eigen::Index c = 0, end = pcs.cols(); c < end; ++c) {
            scores[c] = pcs.coeff(0, c) * multiplier + shift;
        }
    }
...
public:
    ...
    Results run_blocked(const tatami::Matrix<T, IDX>* mat, const X* features, const Block* block) const {
        ...
        if (block == NULL) {
            ...
            compute_scores(temp.pcs, multiplier, shift, output.scores);

        } else {
            MultiBatchPca runner;
            runner.set_rank(1);
            runner.set_scale(scale);
            runner.set_num_threads(nthreads);
            runner.set_return_rotation(true).set_return_scale(scale).set_return_center(true);
            ...
            if (weight_policy == WeightPolicy::EQUAL) {
                runner.set_block_policy(MultiBatchPca::BlockPolicy::WEIGHTED_RESIDUAL);
            } else {
                runner.set_block_policy(MultiBatchPca::BlockPolicy::RESIDUAL_ONLY);
            }
            ...
            // Compute the shift.
            auto nblocks = static_cast<size_t>(NC ? *std::max_element(block, block + NC) : 0) + 1;
            std::vector<int> block_size(nblocks);
            for (IDX j = 0; j < NC; ++j) {
                ++block_size[block[j]];
            }

            Eigen::VectorXd block_weights(nblocks);
            if (weight_policy == WeightPolicy::NONE) {
                std::copy(block_size.begin(), block_size.end(), block_weights.begin());
            } else {
                // TODO: respond to adjustible weights.
                std::fill(block_weights.begin(), block_weights.end(), 1.0);
            }

            double shift = (temp.center * block_weights).sum() / block_weights.sum() / temp.center.size();
            compute_scores(temp.pcs, multiplier, shift, output.scores);
        }
