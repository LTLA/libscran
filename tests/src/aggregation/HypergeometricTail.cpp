...
TEST(HypergeometricTail, Basic) {
...

    {
        ...
        // Check for correct behavior when num_black, num_white < num_drawn.
        {
            ...
            compare_almost_equal(0.6853147, hyper.run(3, 5, 8, 10), /* tol=*/ 0.05);

            // > phyper(13, 15, 18, 20, lower.tail=FALSE)
            compare_almost_equal(0.000500776, hyper.run(13, 15, 18, 20), /* tol=*/ 0.05);
        }
        ...
        // Check for correct boundary case behavior when num_black == num_drawn or num_white == num_drawn.
        {
            ...
            compare_almost_equal(0.0002843443, hyper.run(20, 33, 25, 25), /* tol=*/ 0.01);

            // > phyper(20, 33, 25, 33, lower.tail=FALSE)
            compare_almost_equal(0.1780022, hyper.run(20, 33, 25, 33), /* tol=*/ 0.01);

            // > phyper(20, 25, 25, 25, lower.tail=FALSE)
            compare_almost_equal(1.308459e-06, hyper.run(20, 25, 25, 25), /* tol=*/ 0.01);
        }
    }
...
TEST(HypergeometricTail, EdgeCases) {
...

    {
        ...
        EXPECT_EQ(hyper.run(0, 20, 30, 20), 0);
        EXPECT_TRUE(std::isinf(hyper.run(20, 20, 30, 20)));
