...
public:
    ...
    Results run_blocked(size_t n, const Block* block, const Input* metrics, Buffer* buffer) const {
        if (block) {
            return run_blocked(n, block, compute_block_starts(n, block), metrics, buffer);
        } else {
     ...
     * Values should be integer IDs in \f$[0, N)\f$ where \f$N\f$ is the number of blocks.
     * If a null pointer is supplied, all observations are assumed to belong to the same block.
     * @param starts Vector of start positions for each block,
...
public:
    ...
    Results run_blocked(size_t n, const Block* block, const Input* metrics) const {
        std::vector<double> buffer(n);
        return run_blocked(n, block, metrics, buffer.data());
    }
