    ...
    for (size_t g1 = 0; g1 < ngroups; ++g1) {
        ...
        for (size_t g2 = 0; g2 < ngroups; ++g2) {
            ...
            for (int b = 0; b < nblocks; ++b) {
                double d = means[offset1 + b] - means[offset2 + b];
...
TEST(MarkerLogFC, BlockedMissing) {
    double nan = std::numeric_limits<double>::quiet_NaN();
    int nblocks = 2, ngroups = 4;
    std::vector<double> means{ nan, 0.2, nan, 0.4, nan, 0.6, nan, 0.8 };
